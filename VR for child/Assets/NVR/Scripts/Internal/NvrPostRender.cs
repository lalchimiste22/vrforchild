...
/// and NvrPreRender work together to draw the whole screen in VR Mode.
/// There should be exactly one of each component in any NVR-enabled scene. It
/// is part of the _NvrCamera_ prefab, which is included in
/// _NvrMain_. The NvrViewer script will create one at runtime if the
...
[AddComponentMenu("NVR/Internal/NvrPostRender")]
public class NvrPostRender : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        Reset();
        meshMaterial = new Material(Shader.Find("NVR/UnlitTexture"));
        uiMaterial = new Material(Shader.Find("NVR/SolidColor"));
    ...
    void OnPreCull()
    {
        ...
        float fakeAspect = NvrViewer.Instance.Profile.screen.width / NvrViewer.Instance.Profile.screen.height;
    ...
    void OnRenderObject()
    {
        if (Camera.current != cam || !NvrViewer.Instance.VRModeEnabled)
            return;

        NvrViewer.Instance.UpdateState();

        if (!Application.isEditor && NvrGlobal.supportDtr) return;

        // 编辑器模式使用默认纹理
        RenderTexture stereoScreen = NvrViewer.Instance.GetStereoScreen(0);
        ...
        //{
        //    NvrEye nvreye = eyes[i];
        //    // 左右眼绘制结束
        //    RenderEventType eventType = nvreye.eye == NvrViewer.Eye.Left ? RenderEventType.LeftEyeEndFrame : RenderEventType.RightEyeEndFrame;
        //    // 左右眼绘制结束事件
        //    int eyeTextureId = (int)nvreye.cam.targetTexture.GetNativeTexturePtr();
        //    NvrPluginEvent.IssueWithData(eventType, eyeTextureId);
        //}
        //NvrPluginEvent.IssueWithData(RenderEventType.TimeWarp, NvrViewer.Instance.GetTimeWarpViewNum());
        //}
        
        if ((!NvrViewer.USE_DTR  || useDFT) && NvrGlobal.distortionEnabled)
        {
            // 使用unity畸变绘制
            if (distortionMesh == null || NvrViewer.Instance.ProfileChanged)
            {
    ...
    void OnGUI()
    {
        ...
        bool useDFT = NvrViewer.USE_DTR && !NvrGlobal.supportDtr;
        if ((!NvrViewer.USE_DTR || useDFT) && !NvrGlobal.distortionEnabled && !firstDraw)
        {
            RenderTexture stereoScreen = NvrViewer.Instance.GetStereoScreen(0);
            if (stereoScreen == null)
            {
    ...
    {
        bool vrMode = NvrViewer.Instance.VRModeEnabled;
        if (Application.isEditor)
        {
            ComputeUIMatrix();
        }
        uiMaterial.SetPass(0);
        // 不绘制中间竖线和设置按钮 by caohao
        //DrawSettingsButton();
        //DrawAlignmentMarker();
        if (vrMode)
        {
