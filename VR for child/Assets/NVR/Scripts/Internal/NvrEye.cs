...
public class NvrEye : MonoBehaviour {
  ...
  public NvrStereoController Controller {
    ...
    get {
      ...
      if ((Application.isEditor && !Application.isPlaying) || controller == null) {
        ...
        return transform.parent.GetComponentInParent<NvrStereoController>();
      }
      return controller;
    }
  }

  /// Returns the closest ancestor NvrHead.
  /// @note Uses GetComponentInParent(), so the result will be null if no active ancestor is found.
  public NvrHead Head {
    get {
      return GetComponentInParent<NvrHead>();
    }
  }

  private NvrStereoController controller;
  ...
  public void UpdateStereoValues() {
    ...
    public void UpdateTargetTexture()
    {
        ...
        //int tid = (int)cam.targetTexture.GetNativeTexturePtr();
        //Debug.Log(eyeType + " eye UpdateTargetTexture  textureId = " + tid);
    }
    ...
    private void SetupStereo(bool forceUpdate) {
        ...
        || NvrViewer.Instance.ProfileChanged  // New QR code.
        || cam.targetTexture == null
            && NvrViewer.Instance.GetStereoScreen(eyeType) != null ;  // Need to (re)assign targetTexture.
    if (updateValues) {
