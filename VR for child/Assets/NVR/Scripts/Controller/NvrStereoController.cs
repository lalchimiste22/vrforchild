...
namespace Nvr.Internal
{
    ...
    public class NvrStereoController : MonoBehaviour
    {
        ...
        /// viewing distances (see NvrViewer.cs), or else adjust the stereo multiplier to
  ...
  private NvrHead head;
        ...
        public NvrEye[] Eyes
        {
            ...
            {
#if UNITY_EDITOR
                NvrEye[] eyes = null;  // Local variable rather than member, so as not to cache.
#endif
                if (eyes == null)
                {
        ...
        public NvrHead Head
        {
            ...
            {
#if UNITY_EDITOR
                NvrHead head = null;  // Local variable rather than member, so as not to cache.
#endif
                if (head == null)
                {
        ...
        /// Clear the cached array of NvrEye children, as well as the NvrHead that controls
        /// their gaze.
        /// @note Be sure to call this if you programmatically change the set of NvrEye children
    ...
    head = null;
        ...
        public void AddStereoRig()
        {
            ...
            var eyes = GetComponentsInChildren<NvrEye>(true).Where(eye => eye.Controller == this);
            if (eyes.Any())
            {
                return;
            }
            ...
            if (Head == null)
            {
                var head = gameObject.AddComponent<NvrHead>();
        ...
        void LateUpdate()
        {
            NvrViewer.Instance.UpdateDeviceState();
